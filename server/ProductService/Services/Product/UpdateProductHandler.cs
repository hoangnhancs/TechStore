    ...
    public class UpdateProductHandler : IRequestHandler<UpdateProductCommand, AppResult<ProductDto>>
    {
        ...
        public async Task<AppResult<ProductDto>> Handle(UpdateProductCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.ProductId == null)
                return AppResult<ProductDto>.Failure("Product ID cannot be null or empty", 400);

            var existingProduct = await _unitOfWork.ProductRepository.GetProductWithDetailsAsync(request.ProductId, cancellationToken);
            ...
            try
            {
                ...
                foreach (var image in oldDetailImages)
                {
                ...
                foreach (var img in uploadedDetailImages)
                {
                    existingProduct.DetailImages.Add(new ProductImage
                    {
                        ImageUrl = img.Url,
