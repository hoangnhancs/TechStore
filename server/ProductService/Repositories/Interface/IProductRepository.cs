...
namespace ProductService.Repositories.Interface
{
    public interface IProductRepository : IBaseEFRepository<Product, string>
    {
        ...
        Task<Product?> GetProductWithDetailsAsync(string productId, CancellationToken cancellationToken = default);
        Task<Product?> GetProductForDisplayAsync(string productId, CancellationToken cancellationToken = default);
        Task<List<Product>> GetActiveProductsAsync(DateTime? updatedAfter, CancellationToken cancellationToken = default);
        Task<List<Product>> GetActiveByCategoryOrBrandAsync(int? categoryId, int? brandId, CancellationToken cancellationToken = default);
